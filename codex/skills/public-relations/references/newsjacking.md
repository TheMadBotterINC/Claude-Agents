...
A repeatable workflow the agent can run on demand or on an authorized schedule.
...
6. **Stage owned-channel posts** — draft companion blog, LinkedIn, and X content because it builds the trail journalists check before quoting you; publish only with explicit authorization
...
- **Configured browser or public web search** → search Google News or another current news index for the story → open articles → note the bylines
