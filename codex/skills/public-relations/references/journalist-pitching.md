...
Use configured browser automation when available. Otherwise use public web
search and page-opening tools, and ask the user for authenticated or private
information rather than assuming access. Do not imply that any browser is free
of rate limits.

- Open the journalist's outlet author page → collect the last 5 article headlines + dates
- Open their public X/Twitter profile → note recent topics
- Open their public LinkedIn profile → confirm the current role when visible
