---
name: marketing-plan
description: When the user needs a comprehensive marketing plan for a client, a company they advise, or their own product. Also use when the user mentions "marketing plan," "growth plan," "GTM plan," "go-to-market plan," "AARRR plan," "90-day marketing plan," "12-month marketing roadmap," "fractional CMO plan," or "fCMO plan." Generates an exhaustive 13-section plan structured by AARRR (Acquisition, Activation, Retention, Referral, Revenue), customized to the client's current budget, team, and stage, mapped to future funding milestones, cross-referenced with the 139-idea marketing-ideas library and an embedded 17-section current-state audit rubric, with a full marketing operations stack showing which skills and available connector/API capabilities execute each part. Outputs a Notion-paste-ready markdown document. For positioning and ICP context before planning, see product-marketing. For stage-specific deep work, see onboarding, signup, emails, referrals, pricing.
metadata:
  version: 1.1.0
---
...
On invocation, the skill reads `.agents/marketing-plans/{client-slug}/progress.md` in the current workspace and resumes based on the state machine documented in `references/methodology.md` Step 1.1.2 (fresh → INIT → REVIEW → FINALIZE → finalized). If the user chooses another workspace-local output directory, use it consistently. Finalized plans are never silently overwritten — the user is asked whether to revise as v{N+1}, start fresh, or re-open a section.
...
Save each confirmed section to the progress file as you go. The skill is resumable — if interrupted, invoke `$marketing-plan client-name` again to pick up at the next unfinished section.

### Phase 3 — FINALIZE (compile + verify + publish)

Compile all 13 sections into `final_plan.md`. Run a verification pass: confirm cross-references (marketing-ideas idea numbers, related skills, connector/API capabilities) are accurate; check for machine-specific paths that shouldn't ship; ensure the brand voice matches what was captured in the strategic frame.
...
11. **Marketing operations stack** — Marketing skills + available connector/API capabilities mapped to each AARRR stage. Capability unlocks by funding stage.
...
A small team + an fCMO + the marketing-skills library + configured integrations may increase execution capacity substantially. Treat any comparison with a larger traditional marketing org as a hypothesis that requires client-specific evidence, not as a guaranteed result. The plan must show the available stack explicitly, AARRR-stage by AARRR-stage.
...
9. **The connector/API/tool connections that enable execution** — every move names configured tooling, marks a connector as future/not configured, or explains the manual fallback.
...
- The ops stack section names specific skills plus verified connectors or manual fallbacks per move
...
The final deliverable is a single markdown file: `.agents/marketing-plans/{client-slug}/final_plan.md` (or the workspace-local output directory the user chose).
...
7. **Tooling stack** — What's configured and authorized? Customer.io / Mailchimp / Resend? Shopify / Stripe / App Store Connect? GA4 / Mixpanel / Amplitude? GitHub / Notion / Figma?
