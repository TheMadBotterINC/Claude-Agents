...
```
.agents/marketing-plans/{client-slug}/
...
### Step 1.3 — Pull live data where configured and authorized

If connectors or APIs are configured, available, and authorized for this task, pull:

- **Ahrefs** → domain rating, organic keywords, backlinks, top pages, ref domains (per `$seo-audit` skill)
- **GA4 connector/API** → traffic by channel, conversion events, retention curves
- **Stripe connector/API** → MRR, ARR, churn, plan mix, blended LTV by cohort
- **App Store Connect** (manual or through a configured browser/connector) → install → trial → paid funnel; cohort retention
- **Customer.io connector/API** → flow inventory, send / open / click / unsubscribe rates
- **Shopify** → product page conversion, AOV, repeat rate
- **GitHub connector or CLI** → repos inventory, last commit dates, what's stale
- **Notion** → internal knowledge directory if exposed

Don't ask the user to copy/paste data that can be pulled safely from an authorized connector. If a connector is unavailable, record the gap and ask only for the data needed to proceed.
...
- **Cross-reference check** — every marketing-ideas number (e.g., "idea #17") matches the actual idea in `references/idea-cross-reference.md`. Every related-skill mention either exists in the installed skill set or is documented as an external dependency (see the capability note in `ops-stack-mapping.md`).
- **Connector/API check** — every tool mentioned in Section 11 actually exists in the user's stack and is authorized for the described use (per `research.md`) OR is flagged as "future / not configured."
...
> *"Marketing Plan v1 saved to `.agents/marketing-plans/{client-slug}/final_plan.md`. ~X,XXX words across 13 sections. Ready to paste into Notion or share with the team."*
