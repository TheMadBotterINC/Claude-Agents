...
Copy this file to `.agents/listening-sources.md` in your project and fill in the brackets. The agent reads it when running the [listening workflow](listening.md).
...
### LinkedIn (browser-driven — use a configured, user-authorized session)
...
Search across all platforms. The agent runs these through Reddit, HN, and Bluesky on the [daily loop](listening.md#the-daily-triage-loop).
...
URLs the agent opens with the configured browser capability to scan.
...
- For LinkedIn and X, use a configured browser session only after the user has authorized access; never assume a persistent login
