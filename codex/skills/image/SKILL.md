...
On Codex surfaces with built-in image generation or editing, prefer that capability for raster output. Use this skill to determine the marketing objective, art direction, composition, dimensions, and optimization; ask about external-provider API keys only when the user requests a specific provider or the built-in capability cannot satisfy the task.
...
### 3. Technical Context
- If an external provider is required, do you have API keys for it? (Gemini, Replicate/Flux, Ideogram)
