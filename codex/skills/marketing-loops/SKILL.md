...
These loops are agent-agnostic — the *body* works in any agent. The *scheduling* depends on the current Codex surface and the user's environment:

- **Codex recurring task or monitor** — use the recurring-task, monitoring, or scheduling mechanism available in the current surface. Verify that it exists and confirm the cadence before claiming the loop is scheduled.
- **Cron or CI scheduler** — when the user wants an external scheduler, wrap the loop body as a bounded prompt or script (`0 9 * * 1` for Mondays at 9am, for example). Provide the configuration; do not install or activate it without authorization.
- **No scheduling capability available** — produce the complete loop specification and a runnable handoff for cron, CI, or the user's scheduler. State clearly that it has not been scheduled.
